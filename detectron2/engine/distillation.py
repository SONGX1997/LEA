...
def calculate_feature_distillation_loss(source_features, target_features, loss=None):  # pixel-wise
    ...
    feature_list = ["p2", "p3", "p4", "p5", "p6"]

    # print("num_source_features: ", source_features)
...
def calculate_roi_distillation_loss(soften_results, target_results, cls_preprocess=None, cls_loss=None, bbs_loss=None, temperature=1, soften_proposal=None):
    ...
    num_of_distillation_categories = soften_scores.size()[1]-1
    ...
    tot_classes = target_scores.size()[1]-1
    if cls_loss == 'l2':
        l2_loss = nn.MSELoss(size_average=False, reduce=False)
        ...
        # print("labels: ", labels, labels.shape,"outputs_bkg", outputs_bkg, outputs_no_bgk,"soften_scores.shape[1]", soften_scores.shape[1])
        # print("tot_classes-num_of_distillation_categorie: ", tot_classes, num_of_distillation_categories)
