...
from .distillation import calculate_rpn_distillation_loss, calculate_feature_distillation_loss, calculate_attentive_roi_feature_distillation, calculate_roi_distillation_losses
...
class AMPTrainer(SimpleTrainer):
    ...
    def run_step(self):
        ...
        with autocast(dtype=self.precision):
            # loss_dict, _ , _, _, _ = self.model(data) # forABR
            loss_dict = self.model(data)
            if isinstance(loss_dict, torch.Tensor):
                losses = loss_dict
                loss_dict = {"total_loss": loss_dict}
